...
(function() {
  var Entries, StockList, controller, db, express, insertAndReCalculate;
  ...
  controller.get('/stock', function(req, res) {
    ...
    return StockList.getNumberOfStocksWithName(stockname).then(function(count) {
      ...
      if (count > 0) {
        ...
        return Entries.getOrderedEntriesForStockWithName(stockname).then(function(entries) {
  ...
  controller.post('/editentry', function(req, res) {
    var entry;
    entry = req.body;
    Entries.removeEntryById(entry._id).then(function(err) {
      if (err) {
